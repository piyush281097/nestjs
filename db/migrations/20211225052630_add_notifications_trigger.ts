...
export async function down(knex: Knex): Promise<void> {
  ...
  /**
   * Post user tag Trigger
   */
  await knex.raw(
    'DROP TRIGGER trigger_post_user_tagged_create on tagged_users;',
  );
  await knex.raw('DROP FUNCTION post_user_tagged_create');

  await knex.raw(
    'DROP TRIGGER trigger_post_user_tagged_update on tagged_users;',
  );
  await knex.raw('DROP FUNCTION post_user_tagged_update');

  await knex.raw(
    'DROP TRIGGER trigger_post_user_tagged_delete on tagged_users;',
  );
  ...
  /**
   * Trade Comment Trigger
   */
  await knex.raw(
    'DROP TRIGGER trigger_trade_comment_create on trades_comments;',
  );
  await knex.raw('DROP FUNCTION trade_comment_create');

  await knex.raw(
    'DROP TRIGGER trigger_trade_comment_update on trades_comments;',
  );
  await knex.raw('DROP FUNCTION trade_comment_update');

  await knex.raw(
    'DROP TRIGGER trigger_trade_comment_delete on trades_comments;',
  );
